...
void D3D12nBodyGravity::LoadAssets()
{
    ...
    // Create the pipeline states, which includes compiling and loading shaders.
    {
        ...
        UINT compileFlags = 0;
#endif
	HMODULE hModule = nullptr; // This will break if we're loading from a DLL
	
	
	//nBodyGravityCS
	HRSRC nBodyGravityCSresourceInfo = FindResourceA(hModule, "nBodyGravityCS", "HLSDATA");
	if (!nBodyGravityCSresourceInfo)
	{
		fprintf(stderr, "Could not find nBodyGravityCSresource\n");
	}

	HGLOBAL nBodyGravityCSresource = LoadResource(hModule, nBodyGravityCSresourceInfo);
	if (!nBodyGravityCSresource)
	{
		fprintf(stderr, "Could not load nBodyGravityCSresource\n");
	}

	DWORD nBodyGravityCSresourceSize = SizeofResource(hModule, nBodyGravityCSresourceInfo);

	const char* nBodyGravityCSmessage = (const char*)nBodyGravityCSresource;  
//	ParticleDraw
	HRSRC ParticleDrawresourceInfo = FindResourceA(hModule, "ParticleDraw", "HLSDATA");
	if (!ParticleDrawresourceInfo)
	{
		fprintf(stderr, "Could not find ParticleDrawresource\n");
	}

	HGLOBAL ParticleDrawresource = LoadResource(hModule, ParticleDrawresourceInfo);
	if (!ParticleDrawresource)
	{
		fprintf(stderr, "Could not load ParticleDrawresource\n");
	}

	DWORD ParticleDrawresourceSize = SizeofResource(hModule, ParticleDrawresourceInfo);

	const char* ParticleDrawmessage = (const char*)ParticleDrawresource;   


        // Load and compile shaders.
        ThrowIfFailed(D3DCompile(ParticleDrawmessage, ParticleDrawresourceSize,nullptr,nullptr, nullptr, "VSParticleDraw", "vs_5_0", compileFlags, 0, &vertexShader, nullptr));
        ThrowIfFailed(D3DCompile(ParticleDrawmessage, ParticleDrawresourceSize,nullptr,nullptr, nullptr,  "GSParticleDraw", "gs_5_0", compileFlags, 0, &geometryShader, nullptr));
        ThrowIfFailed(D3DCompile(ParticleDrawmessage, ParticleDrawresourceSize,nullptr,nullptr, nullptr, "PSParticleDraw", "ps_5_0", compileFlags, 0, &pixelShader, nullptr));
        ThrowIfFailed(D3DCompile(nBodyGravityCSmessage, nBodyGravityCSresourceSize,nullptr,nullptr, nullptr,  "CSMain", "cs_5_0", compileFlags, 0, &computeShader, nullptr));
